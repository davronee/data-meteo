...
//        }
        ...
        // Import from Excel file in root folder
//        $excelPath = base_path('mtb@meteo.uz.xlsx');
//        if (file_exists($excelPath)) {
//            try {
//                Excel::import(new MeteoBotImport(), $excelPath);
//                Log::info('MeteoBot stations imported successfully from Excel file');
//            } catch (\Exception $e) {
//                Log::error('Error importing MeteoBot stations from Excel: ' . $e->getMessage());
//            }
//        } else {
//            Log::warning('Excel file not found: ' . $excelPath);
//        }
        ...
        foreach (MeteoBotStations::all() as $station) {
            try {
                $data = Http::withOptions([
                    'verify' => false
                ])->withBasicAuth($station->username, $station->password)->get(
                    'https://export.meteobot.com/v2/Generic/Locate',
                    [
                    ...
                    if ($data->successful()) {
                        ...
                        $meteobot = MeteoBotStations::updateOrCreate(
                            ...
                            [
                                ...
                                'latitude' => $arr[3],
                                'longitude' => $arr[4],
                                'is_has_aq' => (strlen($station->stationid) == 8) ? true : false,
                            ]
                        );
                    }
                } catch (\Exception $ex) {
                ...
                Log::error('ERROR', [$ex->getMessage(), ' ' . $station->sn]);
            }
